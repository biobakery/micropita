...
argp = argparse.ArgumentParser( prog = "MicropitaPaperSupervisedValidationPCoA.py", 
    description = """Validates selection by generating PCoA plots of selection from one data set into another.""" )
...
#Outputfile
argp.add_argument( "strOutFigure", metavar = "OutputFile", help = Constants_Arguments.c_strGenericOutputFigureFileHelp)
...
def _main( ):
    ...
    logging.info("Start MicropitaPaperSupervisedValidationPCoA")
    logging.info("MicropitaPaperSupervisedValidationPCoA. The following arguments were passed.")
    ...
    for sMethod in dictAllSelectionStudies.keys():
        if (sMethod == args.sMetric) and (sMethod in [MicroPITA.c_SVM_CLOSE, MicroPITA.c_SVM_FAR]):
            ...
            if len(setsMetricSelection) < len(setsSampleNames):
                ...
                if (not abndSelectionTable.funcIsPrimaryIdMetadata(args.sPairedMetadata)) or (not abndValidationData.funcIsPrimaryIdMetadata(args.sPairedMetadata)):
                    logging.error("".join(["MicropitaPaperSupervisedValidationPCoA:: tried to validate on a none unique key:",args.sPairedMetadata]))
                    return False
                ...
                if not lsPairedSelected:
                    logging.error("MicropitaPaperSupervisedValidationPCoA:: Did not recieve lsPairedSelected.")
                    return False
                ...
                if not lsSelectedInValidation:
                    logging.error("MicropitaPaperSupervisedValidationPCoA:: Did not recieve lsSelectedInValidation.")
                    return False
                ...
                if len(lsSelectedInValidation) == len(setsMetricSelection):
                    ...
                    #Hold prediction data from the prediction file
                    lsPredictions = list()
                    lsProbabilities = list()

                    #Get prediction file data
                    with open(args.sSVMPrediction,'r') as fHndlInput:
                    ...
                    dictLabels = dict([(sLabel,lsLabels.index(sLabel)) for sLabel in lsLabels])
                    for strSVMSelectionLine in lsSVMFileLines[1:]:
                        lsPredictElements = strSVMSelectionLine.split(Constants.WHITE_SPACE)
                        lsPredictions.append(lsPredictElements[0])
                        lsProbabilities.append(float(lsPredictElements[int(dictLabels[lsPredictElements[0]])+1]))

                    #Modify Probabilities for labels on one side or the other
                    sLeftLabel = lsLabels[0]
                    for iPredictionIndex, sPrediction in enumerate(lsPredictions):
                        if sPrediction == sLeftLabel:
                            lsProbabilities[iPredictionIndex] = lsProbabilities[iPredictionIndex] * -1

                    analysis.funcForceXAxis(lsProbabilities)
                    ...
                    logging.error("MicropitaPaperSupervisedValidationPCoA::Not all samples selected for diversity are in the given sample.")
            else:
                logging.error("MicropitaPaperSupervisedValidationPCoA::Not all samples selected for diversity are in the given sample.")

    logging.info("Stop MicropitaPaperSupervisedValidationPCoA")
